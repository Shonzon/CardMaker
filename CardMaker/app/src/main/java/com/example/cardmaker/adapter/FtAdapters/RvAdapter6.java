...
import com.example.cardmaker.MyCardEdit.McEdit6;
...
public class RvAdapter6 extends RecyclerView.Adapter<RvAdapter6.MyHolder>{

    List<RvModel6> listdata;
    Context mContext;

    public RvAdapter6(Context context, List<RvModel6> listdata) {
    ...
    public MyHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.test_card6,viewGroup,false);
        ...
        public MyHolder(final View itemView) {
                        ...
                        public boolean onMenuItemClick(MenuItem item) {
                            switch (item.getItemId()) {
                                case R.id.add:
                                    ...
                                    RvModel6 data = new RvModel6(pname.getText().toString()
                                            ,designation.getText().toString()
                                            ,pphone.getText().toString(),email.getText().toString()
                                            ,address.getText().toString()
                                    );
                                    myRef.child("seven").setValue(data);
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Intent edit1 = new Intent(mContext, McEdit6.class);

                    FirebaseDatabase firebaseDatabase;
                    DatabaseReference myRef;
                    firebaseDatabase = FirebaseDatabase.getInstance();
                    myRef = firebaseDatabase.getReference("usertemplate").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("seven");
